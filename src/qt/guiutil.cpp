...
{
    // Convert twist:// to twist:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("twist://"))
    {
        uri.replace(0, 12, "twist:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "TWIST.lnk";
}
...
{
    return GetAutostartDir() / "twist.desktop";
}
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("TWIST-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  twist-qt [" + tr("command-line options") + "]                     " + "\n";
